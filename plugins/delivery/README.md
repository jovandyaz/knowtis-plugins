...
| `running-preflight` | Skill (manual-only) | `/delivery:running-preflight` — full local verification with a SHIP/NO-SHIP verdict. Not model-invocable: it runs the whole test suite, so it fires only when you ask. The manual-only gate (`disable-model-invocation`) is honored by Claude Code and Cursor; in Codex/OpenCode/Gemini the skill is model-invocable. |
